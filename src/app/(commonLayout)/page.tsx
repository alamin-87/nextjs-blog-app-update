...
export default async function Home() {
  const featuredPostsPromise = postService.getPostService({ isFeatured: true });
  const postsPromise = postService.getPostService(
    { limit: "3" },
    { revalidate: 10 },
  );
    ...
    <div className="max-w-7xl mx-auto px-4">
      ...
      {featuredPosts?.data?.data && featuredPosts.data.data.length > 0 && (
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {featuredPosts.data.data.slice(0, 2).map((post: Post) => (
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-5">
          {posts?.error?.message ? (
            <p className="text-red-500">{String(posts?.error?.message)}</p>
          ) : null}
